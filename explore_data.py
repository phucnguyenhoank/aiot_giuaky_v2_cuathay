...
# Lấy thông tin từ bản ghi đầu tiên để hiểu cấu trúc
first_record = data[0]
print("\nCác trường trong mỗi bản ghi:")
...
# Lấy mẫu tín hiệu PPG, ECG và Resp từ bản ghi đầu tiên để hiển thị
...
plt.subplot(3, 1, 1)
plt.plot(time_ecg, sample_ecg)
plt.title('ECG Signal (First Record)')
plt.xlabel('Time (s)')
plt.ylabel('Amplitude')

plt.subplot(3, 1, 2)
plt.plot(time_ppg, sample_ppg)
plt.title('PPG Signal (First Record)')
plt.xlabel('Time (s)')
plt.ylabel('Amplitude')

plt.subplot(3, 1, 3)
plt.plot(time_resp, sample_resp)
plt.title('Respiratory Signal (First Record)')
plt.xlabel('Time (s)')
plt.ylabel('Amplitude')
