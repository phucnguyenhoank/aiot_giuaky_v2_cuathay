...
try:
    sample_ppg = first_record['ppg'][0, 0]['v'].flatten()
    sample_fs_ppg = first_record['ppg'][0, 0]['fs'][0, 0]
    sample_ecg = first_record['ekg'][0, 0]['v'].flatten()
    sample_fs_ecg = first_record['ekg'][0, 0]['fs'][0, 0]
    sample_resp = first_record['ref'][0, 0]['resp_sig'][0, 0]['imp'][0, 0]['v'].flatten()
...
for i in range(len(data)):
    try:
        ...
        hr_values = params['hr'].flatten()
        rr_values = params['rr'].flatten()
        pr_values = params['pr'].flatten()
        spo2_values = params['spo2'].flatten()
        
        # Chuyển đổi sang kiểu float và loại bỏ các giá trị không hợp lệ
        hr_values = hr_values.astype(float)
        rr_values = rr_values.astype(float)
        pr_values = pr_values.astype(float)
        spo2_values = spo2_values.astype(float)
...
try:
    all_hr = np.concatenate(all_hr_values)
    all_rr = np.concatenate(all_rr_values)
    all_pr = np.concatenate(all_pr_values)
    all_spo2 = np.concatenate(all_spo2_values)
